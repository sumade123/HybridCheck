                         ...
                         plotLines =
                           function(plottingSettings){
                             ...
                             plot <- ggplot(plotting.frame, aes(x=basepos/1000, y=yvalues)) + geom_line(aes(colour=factor(factors)), show_guide=plottingSettings$Legends, size=0.5) + scale_x_continuous(breaks = scales::pretty_breaks(n = 6), expand = c(0.0125,0)) +
                               ylim(0,100) + 
                               xlab("kb") +
                               ylab("Sequence similarity (%)") + scale_colour_manual(name = "", labels=c(gsub("Rpi-","",combo[1]),gsub("Rpi-","",combo[2]), gsub("Rpi-","",combo[3]),values=c("yellow","purple","cyan")) + theme(legend.background = element_rect(fill = "white")) + (legend.key = element_rect(fill = "transparent")) + theme(panel.grid.major = element_line(size = 0.5)) + theme(panel.grid.minor = element_line(size = 0.5))#+ opts(panel.grid.major = none, panel.grid.minor = none)
...
bars and the NaNs will be dealt with my filling them in black.\n\nTo get rid of them use a lower MosaicScale parameter.")
                             ...
                             if(plottingSettings$Legends == T){
                               ...
                               recg <- rectGrob(gp=gpar(fill="white",col="white"))
                               bars <- arrangeGrob(recg, bars, legendgrob, widths = c(0.0545,1.8,plottingSettings$rgbsize), ncol = 3)#, heights = c(15,15,15))                       
                             }
                             return(bars)
                           }
                         )
