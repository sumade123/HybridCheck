...
bars and the NaNs will be dealt with my filling them in black.\n\nTo get rid of them use a lower MosaicScale parameter.")
                             ...
                             bars <- ggplot(plottingFrame, aes(x = X, y = as.factor(Y))) +
                               ...
                               scale_y_discrete(labels = c(gsub("Rpi-","",ContigNames[3]),gsub("Rpi-","",ContigNames[2]), gsub("Rpi-","",ContigNames[1])))#,expand = c(0, 0)) 
                             ...
                             if(plottingSettings$Legends == T){
                               ...
                               bars <- arrangeGrob(recg, bars, legendgrob, widths = c(0.0545,1,0.38), ncol = 3)#, heights = c(15,15,15))                       
                             }
                             return(bars)
                           }
                         )
