                       ...
                       methods = list(
                         ...
                         plotTriplet = function(plottingSettings){
                           if("Lines" %in% plottingSettings$What && "Bars" %in% plottingSettings$What){
                             ...
                             together <- arrangeGrob(plottedbars, plottedlines, top=0, ncol=1)#, heights=c(1.7,2) 
                         ...
                         plotLines =
                           function(plottingSettings){
                             ...
                             plot <- applyPlottingParams(plot, plottingSettings) + theme(plot.margin = margin(t=0))+ theme(axis.title.y = element_text(size = 15,margin = unit(c(0, 4, 0, 0), "mm")),axis.title.x = element_text(size = 15,margin = unit(c(3.5, 0, 0, 0), "mm")),axis.text.y = element_text(size = 12),axis.text.x = element_text(size = 12)) # title = paste("Sequence Similarity Between Sequences for Triplet ", ContigNames[1], ":", ContigNames[2], ":", ContigNames[3], sep="")
                             recg <- rectGrob(gp=gpar(fill="white",col="white"))
                             lines <- arrangeGrob(recg, plot, widths=units(c(plottingSettings$bl,plottingSettings$br),"inches"),ncol=2)                  
                             return(lines)
                           },
...
bars and the NaNs will be dealt with my filling them in black.\n\nTo get rid of them use a lower MosaicScale parameter.")
                             ...
                             bars <- ggplot(plottingFrame, aes(x = X, y = as.factor(Y))) +
                               ...
                               scale_y_discrete(labels = c(paste(gsub("Rpi-","",ContigNames[3])," (A)",sep=""),paste(gsub("Rpi-","",ContigNames[2])," (B)",sep=""), paste(gsub("Rpi-","",ContigNames[1])," (C)",sep="")))#,expand = c(0, 0)) 
                             ...
                             bars <- bars + theme(axis.title.y=element_blank(),axis.text.x=element_blank(),axis.title.x=element_blank(),axis.ticks.x=element_blank()) + theme(plot.margin=unit(c(5.5, 5.5, 0, 0), "points")) + theme(panel.grid.major = element_line(size = 0.5)) + theme(panel.grid.minor = element_line(size = 0.5))#+ theme(legend.box.margin=margin(10,10,10,10))#bars <- bars + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(colour = "black")) + theme(axis.text.y=element_blank(),axis.title.y=element_blank())##axis.ticks.y=element_blank(),axis.title.y=element_blank(),panel.background=element_blank(),panel.grid.major=element_blank(),panel.grid.minor=element_blank(),plot.background=element_blank()) 
                             
                             if(plottingSettings$Legends == T){
                               legend <- readPNG(system.file("extdata/rgblegend.png", package="HybridCheck"))
                               ...
                               recg <- rectGrob(gp=gpar(fill="white",col="white"))
                               bars <- arrangeGrob(recg, bars, legendgrob, widths = units(c(plottingSettings$tl,plottingSettings$tm,plottingSettings$RgbSize),"inches"), ncol = 3)#, heights = c(15,15,15))                       
                             }
                             return(bars)
                           }
                         )
