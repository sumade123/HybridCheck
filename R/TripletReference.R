...
bars and the NaNs will be dealt with my filling them in black.\n\nTo get rid of them use a lower MosaicScale parameter.")
                             ...
                             bars <- ggplot(plottingFrame, aes(x = X, y = as.factor(Y))) +
                               ...
                               scale_x_continuous(breaks = c(seq(from = 1, to = plottingSettings$MosaicScale, by = plottingSettings$MosaicScale / 10), limits=c(125,FullDNALength-125), labels = c(frame$bpX[seq(from = 1, to = plottingSettings$MosaicScale, by = plottingSettings$MosaicScale / 10)], max(frame$bpX)), expand = c(0.0125,0))) + 
                             ...
                             if(plottingSettings$Legends == T){
                               ...
                               legendgrob2 <- arrangeGrob(recg,legendgrob,ncol=1,nrow=2,heights=c(0.1,plottingSettings$RgbSize))#unit(c(plottingSettings$tl,plottingSettings$tm,plottingSettings$RgbSize),c("in","in"))
